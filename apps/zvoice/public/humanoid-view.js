...
if (canvas && stage) {
  ...
  function sampleHead() {
    ...
    for (let i = 0; i < headCount; i += 1) {
      ...
      // Slightly flatten the rear skull and narrow the lower face.
      if (y < 0.15) x *= 0.78 + Math.max(0, y + 0.65) * 0.16;
      if (x < -0.35) x *= 0.94;
      if (y > 0.6) z *= 1.04;

      // Cut a subtle face plane on the right side to create a readable profile.
      if (x > 0.3) {
    ...
    for (let i = 0; i < noseCount; i += 1) {
      ...
      const x = 0.7 + t * 0.22;
      const y = 0.36 - t * 0.28 + Math.sin(ring) * 0.045;
      const z = Math.cos(ring) * 0.065 * (0.4 + t * 0.6);
      addPoint(x, y, z, 'face', random(0.82, 1.2));
    }

    // Jawline and chin accent.
    const jawCount = Math.floor(pointBudget * 0.07);
    for (let i = 0; i < jawCount; i += 1) {
      const t = Math.random();
      const x = 0.66 - t * 0.9;
      const y = -0.18 - Math.sin(t * Math.PI) * 0.28 + random(-0.035, 0.035);
      const z = random(-0.11, 0.11);
      addPoint(x, y, z, 'edge', random(0.9, 1.4));
    }

    // Eye socket / temple halo.
  ...
  function sampleNeckAndShoulders() {
    ...
    for (let i = 0; i < shoulderCount; i += 1) {
      ...
      const x = -0.12 + side * (0.18 + t * 1.3);
      const y = -1.18 - Math.pow(t, 1.6) * 0.18 + random(-0.045, 0.045);
      const z = random(-0.3, 0.3) * (1 - t * 0.55);
      addPoint(x, y, z, 'shoulder', random(0.55, 0.95));
    }
  }
  ...
  function pointColor(point, depth, pulse) {
    ...
    const warmth = Math.max(0, point.x - 0.12);
    if (warmth > 0.32) return `rgba(206,197,149,${0.12 + depth * 0.4})`;
  ...
  function render(now) {
    ...
    // Bright local core under the neck, mirroring the video reference without using external assets.
