...
| Current branch head compose/start evidence (`02785fa798080f67a337665b39333d49c183c383`) | VERIFIED | `git rev-parse HEAD`; `docker compose -f compose.yml up -d --build --wait`; `node scripts/staging-smoke.mjs` | GitHub Actions/immutable artifact evidence still needs a PR-head run; evidence here is isolated Compose only. |
