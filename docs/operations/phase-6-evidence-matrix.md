...
| Identity-provider and tenant-claim decision record | `scripts/staging-decision-record.json` captures the approved OIDC provider class and claim-mapping reference used by the external staging workflow | `scripts/validate-staging-decision-record.mjs` and `scripts/test/staging-decision-record.test.mjs` | Repository | IMPLEMENTED |
