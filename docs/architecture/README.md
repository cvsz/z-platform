...
| Agent orchestration | `services/agent-orchestrator` | Task lifecycle, queues, tool policy |
...
Clients never receive provider secrets. Agent workloads execute with least privilege. The billing ledger only receives validated usage events and never wallet signing authority.
