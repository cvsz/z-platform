...
type config struct{ compose, project, env, lock, audit, remote, branch, sbomDir string; allowPurge, allowBuild bool; healthTimeout time.Duration }
...
type auditRecord struct {
	SchemaVersion string `json:"schemaVersion"`; OperationID string `json:"operationId"`; Command string `json:"command"`; Profile string `json:"profile"`; Actor string `json:"actor"`; StartedAt time.Time `json:"startedAt"`; CompletedAt time.Time `json:"completedAt"`; BeforeRevision string `json:"beforeRevision,omitempty"`; TargetRevision string `json:"targetRevision,omitempty"`; ImageDigests map[string]string `json:"imageDigests,omitempty"`; Result string `json:"result"`; Error string `json:"error,omitempty"`
}
...
func Run(parent context.Context, args []string, stdout, stderr io.Writer) error {
	...
	if !slices.Contains([]string{"start", "stop", "restart", "status", "health", "logs", "doctor", "build", "update"}, cmd) { return codedError{2, fmt.Errorf("unknown command %q", cmd)} }
	...
	if cmd == "doctor" { err = a.doctor(ctx) } else if cmd == "update" { err = a.locked(cmd, func() error { return a.update(ctx, rest, &rec) }) } else if err = a.preflight(ctx); err == nil {
		switch cmd { case "start": err = a.locked(cmd, func() error { return a.start(ctx, rest) }); case "stop": err = a.locked(cmd, func() error { return a.stop(ctx, rest) }); case "restart": err = a.locked(cmd, func() error { return a.restart(ctx, rest) }); case "build": err = a.locked(cmd, func() error { return a.build(ctx, rest, &rec) }); case "status": err = a.compose(ctx, "ps"); case "health": err = a.health(ctx, rest); case "logs": err = a.logs(ctx, rest) }
	}
	if err == nil { rec.Result = "success" } else { rec.Error = redact(err.Error()) }; return err
}

func defaults(profile string) (config, error) { c := config{compose: "compose.yml", project: "z-platform", lock: ".zctl/operation.lock", audit: ".zctl/audit", remote: "origin", branch: "main", sbomDir: ".zctl/sbom", healthTimeout: 180 * time.Second}; switch profile { case "local": c.env = ".env"; c.allowPurge = true; c.allowBuild = true; case "staging": c.env = ".env.staging"; case "production": c.env = ".env.production"; default: return c, fmt.Errorf("unknown profile %q", profile) }; return c, nil }
...
func (a application) gitOutput(ctx context.Context, args ...string) ([]byte, error) { return a.run.output(ctx, "git", args...) }
func (a application) gitRun(ctx context.Context, args ...string) error { return a.run.run(ctx, a.stdout, "git", args...) }
...
func (a application) start(ctx context.Context, args []string) error { svc, f, e := splitServices(args); if e != nil { return codedError{2, e} }; if e = reject(f, "--wait", "--build"); e != nil { return codedError{2, e} }; if slices.Contains(f, "--build") && !a.cfg.allowBuild { return codedError{4, errors.New("build is forbidden for this profile") } }; if e = a.validate(ctx, svc); e != nil { return e }; c := []string{"up", "-d"}; if slices.Contains(f, "--build") { c = append(c, "--build") }; c = append(c, svc...); if e = a.compose(ctx, c...); e != nil { return e }; if slices.Contains(f, "--wait") { return a.wait(ctx, svc) }; return a.compose(ctx, "ps") }
...
func (a application) build(ctx context.Context, args []string, rec *auditRecord) error {
	...
	svc, flags, err := splitServices(args); if err != nil { return codedError{2, err} }; if err = reject(flags, "--no-cache", "--pull", "--push", "--skip-sbom"); err != nil { return codedError{2, err} }; if err = a.validate(ctx, svc); err != nil { return err }
	if dirty, err := a.gitOutput(ctx, "status", "--porcelain"); err != nil { return err } else if len(strings.TrimSpace(string(dirty))) > 0 { return codedError{2, errors.New("build refused because the working tree is dirty")} }
	rev, err := a.gitOutput(ctx, "rev-parse", "HEAD"); if err != nil { return err }; revision := strings.TrimSpace(string(rev)); rec.TargetRevision = revision
	created := time.Now().UTC().Format(time.RFC3339); source := "https://github.com/cvsz/z-platform"; c := []string{"build", "--build-arg", "OCI_REVISION=" + revision, "--build-arg", "OCI_SOURCE=" + source, "--build-arg", "OCI_CREATED=" + created, "--build-arg", "OCI_VERSION=" + revision[:min(12, len(revision))]}; for _, f := range flags { if f != "--skip-sbom" && f != "--push" { c = append(c, f) } }; c = append(c, svc...); if err = a.compose(ctx, c...); err != nil { return err }
	if slices.Contains(flags, "--push") { if err = a.compose(ctx, append([]string{"push"}, svc...)...); err != nil { return err } }
	images, err := a.output(ctx, "config", "--images"); if err == nil { rec.ImageDigests = map[string]string{}; for _, image := range strings.Fields(string(images)) { inspected, e := a.run.output(ctx, "docker", "image", "inspect", "--format", "{{index .RepoDigests 0}}", image); if e == nil { rec.ImageDigests[image] = strings.TrimSpace(string(inspected)) } } }
	if !slices.Contains(flags, "--skip-sbom") { return a.sbom(ctx, revision, strings.Fields(string(images))) }; return nil
}
func (a application) sbom(ctx context.Context, revision string, images []string) error { if _, err := exec.LookPath("syft"); err != nil { return codedError{3, errors.New("syft is required for SBOM generation; install syft or use --skip-sbom")} }; if a.opts.dryRun { return nil }; if err := os.MkdirAll(a.cfg.sbomDir, 0o700); err != nil { return err }; for _, image := range images { name := strings.NewReplacer("/", "_", ":", "_", "@", "_").Replace(image); path := filepath.Join(a.cfg.sbomDir, fmt.Sprintf("%s-%s.spdx.json", name, revision[:min(12, len(revision))])); f, err := os.OpenFile(path, os.O_CREATE|os.O_TRUNC|os.O_WRONLY, 0o600); if err != nil { return err }; err = a.run.run(ctx, f, "syft", image, "-o", "spdx-json"); closeErr := f.Close(); if err != nil { return err }; if closeErr != nil { return closeErr } }; return nil }

func (a application) update(ctx context.Context, args []string, rec *auditRecord) error {
	branch := a.cfg.branch; restart, stash := false, false
	for i := 0; i < len(args); i++ { switch args[i] { case "--branch": if i+1 >= len(args) { return codedError{2, errors.New("--branch requires a value")} }; i++; branch = args[i]; case "--restart": restart = true; case "--stash": stash = true; default: return codedError{2, fmt.Errorf("unknown update flag %q", args[i])} } }
	if _, err := exec.LookPath("git"); err != nil { return codedError{3, errors.New("git is unavailable")} }
	before, err := a.gitOutput(ctx, "rev-parse", "HEAD"); if err != nil { return err }; rec.BeforeRevision = strings.TrimSpace(string(before))
	dirty, err := a.gitOutput(ctx, "status", "--porcelain"); if err != nil { return err }; hasChanges := len(strings.TrimSpace(string(dirty))) > 0; if hasChanges && !stash { return codedError{2, errors.New("update refused because local changes exist")} }
	stashed := false; if hasChanges { if err = a.gitRun(ctx, "stash", "push", "--include-untracked", "--message", "zctl-update-"+rec.OperationID); err != nil { return err }; stashed = true }
	if stashed { defer func() { _ = a.gitRun(context.Background(), "stash", "pop") }() }
	if err = a.gitRun(ctx, "fetch", "--prune", a.cfg.remote, branch); err != nil { return err }; target := a.cfg.remote + "/" + branch
	...
	if err = a.gitRun(ctx, "merge", "--ff-only", target); err != nil { return err }; after, err := a.gitOutput(ctx, "rev-parse", "HEAD"); if err != nil { return err }; rec.TargetRevision = strings.TrimSpace(string(after))
	if err = a.syncDependencies(ctx); err != nil { return err }; if restart { if err = a.preflight(ctx); err != nil { return err }; return a.compose(ctx, "up", "-d", "--build") }; return nil
}
func (a application) syncDependencies(ctx context.Context) error { if _, err := os.Stat("pnpm-lock.yaml"); err == nil { if _, e := exec.LookPath("pnpm"); e != nil { return codedError{3, errors.New("pnpm-lock.yaml exists but pnpm is unavailable")} }; return a.run.run(ctx, a.stdout, "pnpm", "install", "--frozen-lockfile") }; if _, err := os.Stat("package-lock.json"); err == nil { return a.run.run(ctx, a.stdout, "npm", "ci") }; return nil }
...
func (a application) doctor(ctx context.Context) error { checks := map[string]string{}; for _, d := range []string{"docker", "git", "syft"} { if _, e := exec.LookPath(d); e == nil { checks[d] = "ok" } else { checks[d] = "missing" } }; for k, p := range map[string]string{"composeFile": a.cfg.compose, "environmentFile": a.cfg.env} { if _, e := os.Stat(p); e == nil { checks[k] = "ok" } else { checks[k] = "missing" } }; ok := checks["docker"] == "ok" && checks["git"] == "ok" && checks["composeFile"] == "ok" && checks["environmentFile"] == "ok"; if a.opts.json { _ = json.NewEncoder(a.stdout).Encode(map[string]any{"healthy": ok, "checks": checks}) } else { for _, k := range []string{"docker", "git", "syft", "composeFile", "environmentFile"} { fmt.Fprintf(a.stdout, "%-16s %s\n", k, checks[k]) } }; if !ok { return codedError{3, errors.New("doctor checks failed")} }; return nil }
...
func checksum(v string) string { sum := sha256.Sum256([]byte(v)); return hex.EncodeToString(sum[:]) }
func (a application) help() { fmt.Fprintln(a.stdout, "zctl [global flags] <start|stop|restart|build|update|status|health|logs|doctor|version>") }
